...
def load_filtered_data(input_file):
    ...
    with input_file.open(
        ...
        for line_num, line in enumerate(
            ...
            obj["root_id"] = root_id
            obj["language_name"] = language
            obj["category"] = category
            obj["prompt_translated_lang"] = prompt
...
def load_processed_keys(
    ...
    with output_file.open(
        ...
        for line in f:
            ...
            if category is None:
                category = (obj.get("eval") or {}).get("category")
...
class ResponseGenerator:
    ...
    def __init__(
        ...
        # Only pass quantization_config when we actually want to force
        # bitsandbytes on an unquantized checkpoint. Passing it as an
        # explicit None here would suppress transformers' own auto-detection
        # of a pre-quantized checkpoint's embedded quantization_config (e.g.
        # AWQ), causing it to silently load quantized tensors as if the
        # model were unquantized and randomly reinitialize the real weights.
        extra_kwargs = (
    ...
    def format_prompts(
        ...
        for prompt in prompts:
            ...
            try:
                ...
                raise RuntimeError(
                    f"Cannot apply the chat template for {self.model_name}. "
                    "Generation stopped to preserve the system instruction."
                ) from exc
    ...
    def generate_batch(
        ...
        # Reject overlong prompts explicitly instead of silently cutting text.
        ...
        if inputs["attention_mask"].sum(dim=1).max().item() + MAX_NEW_TOKENS > max_input_length:
            raise ValueError(
                f"Prompt plus {MAX_NEW_TOKENS} completion tokens exceeds "
                f"model context limit {max_input_length}; no prompt was truncated."
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Load the model in 4-bit (bitsandbytes NF4) instead of "
            "full bf16 precision. Needed for very large models that "
            "don't fit unquantized on the available GPUs."
        ),
    )
